...
The Rust backend spawns local provider CLIs (`opencode`, `claude`, `codex`, `gemini`, `kimi`, `pi`, `kiro`, `aider`, `muse`), parses their JSON event streams, runs a turn-based state machine, monitors process resources, and tracks git changes. The React frontend renders the conversation, status, and controls.
